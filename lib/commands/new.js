...
var repositories = {
  landing: {
    landing_bourbon: 'https://github.com/Pavel-Mishalov/miracle-cli.git',
    landing_foundation: 'https://github.com/zurb/foundation-zurb-template.git',
    landing_bootstrap: 'https://github.com/bassjobsen/empty-bootstrap-project-gulp.git'
  },
  wp_blog: {
    wp_blog_bourbon: 'https://github.com/Pavel-Mishalov/wp-bourbon-template.git',
    wp_blog_foundation: 'https://github.com/Pavel-Mishalov/wp-bourbon-template.git',
    wp_blog_bootstrap: 'https://github.com/Pavel-Mishalov/wp-bourbon-template.git'
  }
}
...
module.exports = function(args, options, callback, ee) {
  ...
  function preflight(cb) {
    if (isRoot()) {
      console.log(util.mascot('landing', util.messages.noRoot));
      process.exit(1);
    }
  ...
  function prompt(cb) {
    inquirer.prompt(util.questions(options), function(answers) {
      ...
      template = answers.template_front || options.template_front || answers.template_back || options.template_back || 'unspecified';
  ...
  function gitClone(cb) {
    var repo = framework === 'landing'
      ? repositories.landing[template]
      : repositories.wp_blog[framework];
